...
import { registerCommand, registerTextEditorCommand } from './host';
...
function loadTextEditorCommand(requireContext, nameRegex, commandCreator, context: ExtensionContext) {
  requireContext.keys().forEach(fileName => {
    const clearName = fileName
      // Remove the "./" from the beginning
      .replace(/^\.\//, '')
    ...
    try {
      ...
      const cmdInstance: Command = new Cmd();
      logger.debug(`register command "${commandOption.name}" from "${fileName}"`);
      registerTextEditorCommand(context, commandOption.id, cmdInstance.run, cmdInstance);
    } catch (error) {
      logger.error(error, `load command "${fileName}"`);
    }
  });
